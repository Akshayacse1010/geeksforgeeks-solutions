...
class Solution
{
    static void rec(HashSet<Integer>leaf ,HashSet<Integer>nodes,Node node){
        if(node==null){
            return;
        }
        nodes.add(node.data);
        ...
        rec(leaf,nodes,node.left);
        rec(leaf,nodes,node.right);
    }
    public static boolean isDeadEnd(Node root)
    {
       ...
       rec(leaf,nodes,root);
       for(int x : leaf){
           if(nodes.contains(x+1) && nodes.contains(x-1)){
               return true;
           }
       }
       return false;
